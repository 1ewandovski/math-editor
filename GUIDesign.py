...
class CentralWidget(QtWidgets.QWidget):
    ...
    def init_UI(self):
        ...
        self.instructtable.setFixedSize(800,50)
        #self.id_item_table.setFixedSize(800,200)
...
class My_Main_window(QtWidgets.QMainWindow):
    def __init__(self,parent=None):
        ...
        self.feed_text=""
        #self.text="\int_1^\infty f(x)\mathrm{d}x"
        self.text_stack=[]
        self.feed_stack=[]        
        self.initUI()
    def initUI(self):
        ...
        self.feed_Act = QtWidgets.QAction(QIcon('img/line_feed.jpg'), '&换行', self)       
        self.feed_Act.setShortcut('Ctrl+a')
        self.feed_Act.setStatusTip('line feed')
        self.feed_Act.triggered.connect(self.line_feed)
        ...
        self.Nsqrt_Act.triggered.connect(self.Nsqrt_input)
        ...
        #self.menu2.addAction(self.feed_Act)
        ...
        self.toolbar.addAction(self.feed_Act)
        ...
        self.setWindowTitle('Galois\' Math Editor v1.2.0')
    ...
    def initial_plot_(self):
        ...
        # 获取绘图并绘制
        self.fig = plt.figure()
        self.fig.text(0.1,0.4,self.feed_text+'$'+self.text+'$',fontsize=20,color="black")
        #self.fig.show()
    ...
    def plot_(self):
        ...
        self.fig.clear()
        #fig=plt.figure()
        #fig.text(0.1,0.4,self.feed_text+'$'+self.text+'$',fontsize=20,color="black")     
        self.fig.text(0.1,0.4,self.feed_text+'$'+self.text+'$',fontsize=20,color="black") 
    ...
    def index_To_word(self,index):
        if self.mode_id == 0:
            ...
            if index not in self.dic.caculas_dic:
                return ' '+index+' '
            else:
                return self.dic.caculas_dic[index]
        elif self.mode_id == 2:
            if index not in self.dic.set_dic:
                return ' '+index+' '
            else:
                return self.dic.set_dic[index]
        elif self.mode_id == 3:
            if index not in self.dic.latter_dic:
                return ' '+index+' '
            else:
                return self.dic.latter_dic[index]
        elif self.mode_id == 4:
            if index not in self.dic.relation_dic:
                return ' '+index+' '
            else:
                return self.dic.relation_dic[index]
        elif self.mode_id == 5:
            if index not in self.dic.func_dic:
                return ' '+index+' '
            else:
    ...
    def Nsqrt_input(self):
        ...
        self.renew_instruction()
    def line_feed(self):
        self.feed_stack.append(self.feed_text)
        self.feed_text=self.feed_text+'$'+self.text+'$ \n'
    ...
    def input_confirm(self):
        ...
        if self.state_id == 0:
            ...
            if self.fraction_symbol == 1:
                self.text_stack.append(self.text)
                self.text=self.text+r'\dfrac{'+self.index_To_word(index)+'}'
                self.fraction_symbol = 0
                self.ins_signal = 3
            else:
                self.text=self.text+'{'+self.index_To_word(index)+'}'
    ...
    def pop_stack(self):        
        if len(self.text_stack) > 0:
            ...
            if self.text == "Hello World!":
                self.feed_text=self.feed_stack.pop() 
                self.text = self.text_stack.pop()                           
        else:
            self.text = "Hello World!"
            self.ins_signal=7
